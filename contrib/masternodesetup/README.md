...
Remember to do `nestegg-cli getblockcount` to check if VPS catching blocks till it synced with chain, if not follow this procedure:

* Go to your wallet-qt and check peers list (tools - peers list) and select one ip from the list. With that ip do the follow command at VPS `nestegg-cli addnode "ip" onetry`

      Example:
		  nestegg-cli addnode 45.32.144.158 onetry
    
* Check now if VPS already downloading blocks with the command `nestegg-cli getblockcount`, and if yes give it time now to catch last block number 
