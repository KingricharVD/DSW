...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected EGG and zEGG supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zEGGsupply']:
                assert_equal(node['zEGGsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check EGG and zEGG supply at the beginning
        # ------------------------------------------
        ...
        # EGG supply: block rewards minus burned fees for minting
        ...
        # Verify that EGG and zEGG supplies were properly updated after the spends and reorgs
